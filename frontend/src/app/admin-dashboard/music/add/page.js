...
export default function AddMusic() {
  ...
  const steps = [
    ...
    { id: 2, title: 'Audio Files', description: 'Upload audio and thumbnail files' },
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-gray-800 rounded-lg p-6 w-full">
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <div>
                <label className="block text-white mb-2">Audio File <span className="text-red-500">*</span></label>
                <input
                  type="file"
                  name="audio_file"
                ...
                {files.audio_file && (
                  <p className="text-green-400 text-sm mt-1">
                    Selected: {files.audio_file.name} ({(files.audio_file.size / 1024 / 1024).toFixed(2)} MB)
                  </p>
                )}
                <p className="text-gray-500 text-xs mt-1">
                  Upload MP3, WAV, or other audio formats. Duration will be auto-calculated.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-500 text-xs mt-1">
                  Optional: Upload album artwork or cover image. Recommended size: 1000x1000 or square aspect ratio.
                </p>
              </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-300 text-sm">
                        Reading audio metadata to determine duration...
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <p className="text-gray-500 text-xs mt-1">
                    {calculatingDuration
                      ...
                      : files.audio_file && formData.duration
                        ? `✓ Duration calculated: ${Math.floor(formData.duration / 60)}m ${formData.duration % 60}s`
                        : formData.duration
                          ? `Current: ${Math.floor(formData.duration / 60)}m ${formData.duration % 60}s`
                          : "Upload an audio file to auto-calculate duration"}
                  </p>
                </div>
              </div>
              ...
              {files.audio_file && (
                <div className="bg-green-900/20 border border-green-500/30 p-4 rounded">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-green-400 font-semibold mb-1">Audio File Uploaded</p>
                      <p className="text-gray-300 text-sm">
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div className="bg-gray-700/50 p-6 rounded">
                <h3 className="text-white font-semibold mb-4">Review Your Music</h3>
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-400"><strong>Album:</strong> {formData.album || 'Not specified'}</p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-400"><strong>Duration:</strong> {formData.duration ? `${Math.floor(formData.duration / 60)}m ${formData.duration % 60}s` : 'Not set'}</p>
                  </div>
                </div>
                <div className="mt-4">
                  <h4 className="text-gray-300 font-medium mb-2">Files</h4>
                  <p className="text-sm text-gray-400 mb-1">
                    <strong>Audio:</strong> {files.audio_file ? files.audio_file.name : 'No file selected'}
                  </p>
                  <p className="text-sm text-gray-400">
                    <strong>Thumbnail:</strong> {files.thumbnail_file ? files.thumbnail_file.name : 'No thumbnail'}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-blue-400 font-semibold mb-1">Ready to Submit</p>
                    <p className="text-gray-300 text-sm">
                      Review your music information above and click "Add Music" to submit.
                    </p>
                  </div>
                </div>
              </div>
