  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white">
        <Navbar />
        <div className="pt-24 px-4 sm:px-8 py-12 flex items-center justify-center">
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-28 pb-6 px-4 sm:px-6 md:px-8 md:pb-8">
        ...
        <div className="mb-6 md:mb-8">
          ...
          <p className="text-gray-400 text-sm sm:text-base md:text-lg">Manage your account and preferences</p>
        </div>

        <div className="flex flex-col md:flex-row gap-4 md:gap-8">
          {/* Sidebar - horizontal scroll on mobile, vertical on md+ */}
          <div className="md:w-64 flex-shrink-0">
            <div className="bg-gray-900/50 rounded-xl p-3 md:p-4 md:sticky md:top-24">
              <div className="flex md:flex-col gap-2 overflow-x-auto scrollbar-hide pb-1 md:pb-0">
                {sections.map((section) => (
                  <button
                    key={section.id}
                    onClick={() => setActiveSection(section.id)}
                    className={`flex-shrink-0 md:w-full text-left px-3 md:px-4 py-2 md:py-3 rounded-lg md:mb-2 transition flex items-center gap-2 md:gap-3 whitespace-nowrap ${activeSection === section.id
                      ? 'bg-blue-600 text-white'
                      : 'text-gray-400 hover:bg-gray-800 hover:text-white'
                      }`}
                  >
                    <span className="text-base md:text-xl">{section.icon}</span>
