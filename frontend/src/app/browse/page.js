  ...
  const loadContent = async () => {
    ...
    try {
      ...
      if (type === 'MOVIE') {
        ...
        setMovies(selectedGenre ? movieData.filter(m => m.genre.toLowerCase().includes(selectedGenre.toLowerCase())) : movieData);

        const genreData = await api.getGenres(token, 'video');
        setGenres(genreData.genres || []);
      } else if (type === 'SERIES') {
        const seriesData = await api.getVideos(token, 'SERIES');
        setSeries(selectedGenre ? seriesData.filter(s => s.genre.toLowerCase().includes(selectedGenre.toLowerCase())) : seriesData);
